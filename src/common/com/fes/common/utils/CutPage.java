...
public class CutPage implements Serializable {
	...
	private static final long serialVersionUID = -1000077279595514554L;

	private static Logger log = Logger.getLogger(CutPage.class);
	 ...
	 */
	public void init(HttpServletRequest request, String sql, int pageSize, int groupSize, int pageNo, String url) {
		// 上一页、下一页跳转路径
		if (url != null) {
			this.url = url;
		}
		...
		this.size = parseInt(request.getSession().getAttribute("page_all_size") + "", 0);
		if (this.curPageNo % this.groupSize == 0 || (request.getSession().getAttribute("cur_group") != null && parseInt("" + request.getSession().getAttribute("cur_group"), 1) != curGroup)
				|| this.size == 0 || request.getParameter("reload") != null) {

			request.getSession().setAttribute("cur_group", curGroup);

			if (pageNo > 0 && request.getSession().getAttribute("page_sql") != null) {
			...
			List list = getPageData(sql, (this.curPageNo / this.groupSize) * this.pageSize * this.groupSize, this.pageSize * this.groupSize);
			...
			this.setShowList((List) request.getSession().getAttribute("page_cur_list"));// 设置页面上的显示数据
		}
	}
	 ...
	 *            如是在aciton里面调用，并且action是继承自DispatherAction的话最好传入完整的路径
	 */
	public void init(HttpServletRequest request, String sql, int pageSize, int groupSize, String url) {
		// 当前第几页
	 ...
	 *            如是在aciton里面调用，并且action是继承自DispatherAction的话最好传入完整的路径
	 */
	public void init(HttpServletRequest request, String sql, int pageSize, int groupSize, int pageNo) {
	 ...
	 * 
	 * 这种方法在jsp页面上使用很方便，但是在servlet或action里面使用时，要达到减少数据库访问的目的，则需先判断是否执行查询数据库操作，
	 * 如下使用：
	 * 
	 ...
	 *            如是在aciton里面调用，并且action是继承自DispatherAction的话最好传入完整的路径
	 */
	public void init(HttpServletRequest request, List allList, int pageSize, String url) {
	...
	public String getPageNavigation() {
		...
		if (size > pageSize) {
			/*
			 * if (url != null && !"".equals(url)) { if (url.indexOf("?") > -1)
			 * { // 如果url中已经包含了其他的参数,就把curPageNo参数接在后面 url += "&"; } else { //
			 * 如果url中没有别的参数 url += "?"; } // 生成一个提交页面的函数 pageNavigation +=
			 * "<script>"; pageNavigation += "function gotoPage(page_num){";
			 * pageNavigation += "location.href='" + url + "pageNo='+page_num;";
			 * pageNavigation += "}</script>"; }
			 */
			pageNavigation += "每页" + pageSize + "条&nbsp;&nbsp;" + "当前第<select name='pageNo' id='pageNo' onchange=\"gotoPage('" + url + "',this.value)\">";
			int curPageNos = size % pageSize == 0 ? size / pageSize : size / pageSize + 1;
			for (int i = 0; i < curPageNos; i++) {
				if (i == curPageNo) {
					pageNavigation += "<option value='" + i + "' selected>" + (i + 1) + "</option>";
				}
				else {
					pageNavigation += "<option value='" + i + "'>" + (i + 1) + "</option>";
				}
			}
			pageNavigation += "</select>页&nbsp;&nbsp;共" + curPageNos + "页&nbsp;&nbsp;";
			// 如果不是第一页,导航条将包含"首页"和"上一页"的连接
			if (curPageNo > 0) {
				pageNavigation += "[<a href=\"javascript:void(0);\" onclick=\"gotoPage('" + url + "',0);return false;\">首页</a>]&nbsp;" + "[<a href=\"javascript:void(0);\" onclick=\"gotoPage('" + url
			...
			if (curPageNo < curPageNos - 1) {
				pageNavigation += "[<a href=\"javascript:void(0);\" onclick=\"gotoPage('" + url + "'," + (curPageNo + 1) + ");return false;\">下一页</a>]&nbsp;"
						+ "[<a href=\"javascript:void(0);\" onclick=\"gotoPage('" + url + "'," + (curPageNos - 1) + ");return false;\">末页</a>]";
			}
			else {
	...
	private void setShowList(List list) {
		log.info("$$totalSize=" + this.size + "; curPageNo=" + this.curPageNo + "; pageSize=" + this.pageSize + "; groupSize=" + this.groupSize);
		if (list == null) {
			list = new ArrayList();
		}
			...
			if (groupSize <= 1) {
				...
				if (pageSize * (curPageNo + 1) > list.size()) {
					if (pageSize * curPageNo > list.size()) {
						this.showList = list.subList(list.size() - pageSize, list.size());// 返回最后一页的数据
					}
					else {
						this.showList = list.subList(pageSize * curPageNo, list.size());
					}
				}
				else {
					this.showList = list.subList(pageSize * curPageNo, pageSize * (curPageNo + 1));
				}
			}
			else {
				if (pageSize * ((curPageNo % groupSize) + 1) > list.size()) {
					this.showList = list.subList(pageSize * (curPageNo % groupSize), list.size());
				}
				else {
					this.showList = list.subList(pageSize * (curPageNo % groupSize), pageSize * ((curPageNo % groupSize) + 1));
				}
			}
		}
	}
		...
		case 2:
			if (sql.indexOf("where") > 0) {
				if (sql.indexOf("order by") > 0) {
					return sql.substring(sql.indexOf("where"), sql.indexOf("order by") - 1);
				}
				else {
	 ...
	 * @return
	 */
	private List getPageData(final String sql, final int firstResult, final int maxResults) {
		log.debug("$$sql=" + sql + "; firstResult=" + firstResult + "; maxResults=" + maxResults);
		try {
			JdbcTemplate jt = getSpringJdbcTemplate();
			return queryForList(sql + " limit " + firstResult + "," + maxResults, jt);
		}
		catch (Exception e) {
			e.printStackTrace();
		}
